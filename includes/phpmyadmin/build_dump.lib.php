...
if (!defined('PMA_BUILD_DUMP_LIB_INCLUDED')){
     ...
     * @param   string   the datamysqli_fetch_array
    ...
    function PMA_getTamysqli_fetch_arraye, $crlf, $error_url)

    {
        ...
        if (PMA_MYSQL_INT_VERSION >= 32321) {
            ...
            if ($use_backquotes) {
                ...
                mysql_query_to_mysqli_query('SET SQL_QUOTE_SHOW_CREATE = 1');

            } else {

                mysql_query_to_mysqli_query('SET SQL_QUOTE_SHOW_CREATE = 0');

            }

            $result = mysql_query_to_mysqli_query('SHOW CREATE TABLE ' . PMA_backquote($db) . '.' . PMA_backquote($table));
        ...
        $result        = mysql_query_to_mysqli_query($local_query) or PMA_mysqlDie('', $local_query, '', $error_url);
