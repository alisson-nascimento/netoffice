...
mysqli_connect
    ...
    // If zlib output compression is set in the php configuration filemysqli_connect
     ...
     */
        ...
        if ($dblist_cnt) {
            ...
            for ($i = 0; $i < $dblist_cnt; $i++) {
                ...
                if ($is_show_dbs && ereg('(^|[^\])(_|%)', $dblist[$i])) {
                    ...
                    $rs          = mysql_query_to_mysqli_query($local_query, $dbh);

                    // "SHOW DATABASES" statement is disabled

                    if ($i == 0

                        && (mysql_error() && mysqli_errno() == 1045)) {
        ...
        // databases in the "mysql" db
            ...
            $rs         = mysql_query_to_mysqli_query($auth_query, $dbh); // Debug: or PMA_mysqlDie('', $auth_query, FALSE);
        ...
        if (!$dblist_cnt && @mysql_numrows($rs)) {
            ...
            if ($row['Select_priv'] != 'Y') {
                ...
                $rs          = mysql_query_to_mysqli_query($local_query, $dbh); // Debug: or PMA_mysqlDie('', $local_query, FALSE);
...

    {
        ...
        $result = mysql_query_to_mysqli_query('SELECT COUNT(*) AS num FROM ' . PMA_backquote($db) . '.' . PMA_backquote($table));
...
<div align="<?php echo $GLOBALS['cell_align_left']; ?>">
...

    {
        ...
        if (file_exists($filename)) {
            ...
            $word_cnt  = teste_count($word_list);
